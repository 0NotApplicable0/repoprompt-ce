...
struct AgentTaskRoutingCandidateBuilder {
    ...
    func build(
        ...
        for resolution in resolutions where roles.contains(resolution.role) && !resolution.overrideUnavailable {
        ...
        guard (2 ... 4).contains(order.count) else { throw BuildError.insufficientDistinctTargets }
