...
final class MCPAgentControlToolProvider: MCPAppToolProviding {
    ...
    private func agentExploreTool() -> Tool {
        ...
        return runtime.tool(
            ...
            **Operations**: start | poll | wait | cancel
            ...
            - `wait`: Block until the first referenced explore run finishes or needs input. Omit `timeout` to use the default unless the user requests otherwise. `timeout=0` behaves like poll.
    ...
    private func agentRunTool() -> Tool {
        ...
        return runtime.tool(
            ...
            **Parallel agents**: When launching multiple agents in parallel, always use `detach: true` so each `start` returns immediately without blocking. You can then `wait` or `poll` each `session_id` independently. When there is no useful independent work, prefer `wait` over repeated polling and omit the timeout unless the user requests otherwise.
