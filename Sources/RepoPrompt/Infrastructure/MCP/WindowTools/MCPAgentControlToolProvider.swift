...
final class MCPAgentControlToolProvider: MCPAppToolProviding {
    ...
    private func agentExploreTool() -> Tool {
        ...
        return runtime.tool(
            ...
            **Operations**: start | poll | wait | cancel
            ...
            - `wait`: Block until the first referenced explore run finishes or needs input. For routine idle monitoring, omit `timeout` to use the default. Use `poll` for a deliberate status check or a shorter timeout for a concrete coordination need. `timeout=0` behaves like poll.
    ...
    private func agentRunTool() -> Tool {
        ...
        return runtime.tool(
            ...
            **Parallel agents**: When launching multiple agents in parallel, always use `detach: true` so each `start` returns immediately without blocking. You can then `wait` or `poll` each `session_id` independently. Do useful independent work before blocking. For routine idle monitoring, prefer the default `wait` over repeated polling; deliberate status checks and shorter waits remain appropriate for concrete coordination needs.
