...
final class ContextBuilderWatchdogStabilityTests: XCTestCase {
    ...
    func testProductionAllowsFortyFiveMinutesOfSilentReasoning() async {
        let state = ContextBuilderFollowUpFinalizationState(startedAt: 0)
        for elapsed: TimeInterval in [600, 2700, 3599] {
            let snapshot = await state.timeoutSnapshot(at: elapsed, configuration: .production)
            XCTAssertNil(snapshot, "Healthy silent reasoning was timed out at \(elapsed)s")
        }
    }

    func testProductionStillBoundsSilentStreamsAtOneHour() async {
        let state = ContextBuilderFollowUpFinalizationState(startedAt: 0)
        let snapshot = await state.timeoutSnapshot(at: 3600, configuration: .production)
        XCTAssertEqual(snapshot?.kind, .inactivity)
        XCTAssertEqual(snapshot?.lastKnownSubphase, ContextBuilderMCPProgressPhase.streaming.displayName)
    }

    func testProviderStopRestoresTenMinuteFinalizationBudget() async {
        let state = ContextBuilderFollowUpFinalizationState(startedAt: 0)
        let update = await state.record(.init(kind: .providerStopObserved), at: 2700)
        XCTAssertTrue(update.shouldTransitionToFinalization)
        let beforeDeadline = await state.timeoutSnapshot(at: 3299, configuration: .production)
        let atDeadline = await state.timeoutSnapshot(at: 3300, configuration: .production)
        XCTAssertNil(beforeDeadline)
        XCTAssertEqual(atDeadline?.kind, .inactivity)
        XCTAssertEqual(atDeadline?.inactiveFor, 600)
        XCTAssertEqual(atDeadline?.lastKnownSubphase, ContextBuilderMCPProgressPhase.messageFinalization.displayName)
    }

    func testOverallDeadlineWinsDespiteRecentStreamActivity() async {
        let state = ContextBuilderFollowUpFinalizationState(startedAt: 0)
        _ = await state.record(.init(kind: .streamActivity), at: 14399)
        let snapshot = await state.timeoutSnapshot(at: 14400, configuration: .production)
        XCTAssertEqual(snapshot?.kind, .overall)
        XCTAssertEqual(snapshot?.inactiveFor, 1)
    }

    func testExplicitLegacyConfigurationKeepsItsStreamingBudget() async {
        ...
        let state = ContextBuilderFollowUpFinalizationState(startedAt: 0)
        let beforeDeadline = await state.timeoutSnapshot(at: 9, configuration: configuration)
        let atDeadline = await state.timeoutSnapshot(at: 10, configuration: configuration)
        XCTAssertNil(beforeDeadline)
        XCTAssertEqual(atDeadline?.kind, .inactivity)
    }

    func testWatchdogBookkeepingDoesNotManufactureStreamActivity() async {
        let configuration = ContextBuilderFollowUpFinalizationConfiguration(
            overallTimeout: 100,
            inactivityTimeout: 10,
            checkInterval: 1,
            streamingInactivityTimeout: 20
        )
        let state = ContextBuilderFollowUpFinalizationState(startedAt: 0)
        _ = await state.record(.init(kind: .streamActivity), at: 5)
        _ = await state.record(.init(kind: .finalizationWatchdogArmed), at: 24)
        let snapshot = await state.timeoutSnapshot(at: 25, configuration: configuration)
        XCTAssertEqual(snapshot?.kind, .inactivity)
        XCTAssertEqual(snapshot?.inactiveFor, 20)
        XCTAssertEqual(snapshot?.lastKnownSubphase, ContextBuilderMCPProgressPhase.streaming.displayName)
    }
